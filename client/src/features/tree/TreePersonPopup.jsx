...
import { QUICK_ADD_RELATIONS, getMoreRelationTypes, inferLineForRelation, relationNeedsLine } from "./treeAddRelativePresets.js";
...
/**
 * Всплывающее меню по клику на человека в древе: карточка / добавить родственника.
 */
export function TreePersonPopup({
  ...
  const moreTypes = getMoreRelationTypes();
      ...
      {onStartAddRelative ? (
        <>
          ...
          {isVirtual ? (
            ...
            <div className="tree-person-popup__grid">
          ...
          {!isVirtual && moreTypes.length > 0 ? (
            <details className="tree-person-popup__more">
              <summary>Ещё типы связи</summary>
              <div className="tree-person-popup__grid tree-person-popup__grid--more">
                {moreTypes.map((t) => (
                  <Button
                    key={t}
                    type="button"
