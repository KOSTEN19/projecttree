...
function makeFamilyPinIcon({ count, initials, cityShort, kind }) {
  ...
  const pinHtml = makeFamilyPinHtml({ count, initials, cityShort, kind });
  return L.divIcon({
...
export default function MapPage() {
  ...
  const fitBoundsIfNeeded = useCallback(
    (map) => {
      ...
      if (!any) return;
      try {
        map.fitBounds(bounds, { padding: [72, 72], maxZoom: 11, animate: true });
      } catch {
        /* ignore */
      }
    },
    ...
    (map) => {
      ...
      const layer = L.geoJSON(fc, {
        ...
        onEachFeature(feature, marker) {
          ...
          marker.on("mouseover", () => {
            marker.getElement()?.classList.add("fam-pin-marker--hover");
            const popupHtml =
              count > 1
          ...
          marker.on("mouseout", () => {
            marker.getElement()?.classList.remove("fam-pin-marker--hover");
            try {
              map.closePopup();
            } catch {
              /* ignore */
  ...
  useEffect(() => {
    ...
    function tryInit() {
      ...
      const basemapKey = getDarkSnapshot() ? "dark" : "light";
      map = L.map(el, {
        zoomControl: true,
      ...
      tileLayerRef.current = L.tileLayer(BASEMAP[basemapKey].url, {
        attribution: BASEMAP[basemapKey].attribution,
        maxZoom: 19,
      }).addTo(map);
  ...
  useEffect(() => {
    ...
    const nextKey = isDark ? "dark" : "light";
    if (basemapKeyRef.current === nextKey) return;
    if (tileLayerRef.current) {
      map.removeLayer(tileLayerRef.current);
      tileLayerRef.current = null;
    }
    tileLayerRef.current = L.tileLayer(BASEMAP[nextKey].url, {
      attribution: BASEMAP[nextKey].attribution,
      maxZoom: 19,
    }).addTo(map);
    basemapKeyRef.current = nextKey;
  }, [isDark, leafletMap]);
          ...
          <div className="pointer-events-auto flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="fam-map-chrome-panel max-w-md rounded-xl border border-border/70 bg-card/90 px-4 py-3 shadow-lg ring-1 ring-black/5 backdrop-blur-md dark:bg-card/85 dark:ring-white/10">
              ...
              <p className="text-muted-foreground mt-1 max-w-sm text-sm leading-snug">
                Подложка OpenStreetMap/CARTO. Точки по городам из справочника. Клик по маркеру — список или карточка.
                Цвет маркера подстраивается под тему интерфейса.
              </p>
